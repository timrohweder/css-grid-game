...
const GameWrapper = styled.div`
  --grid-gap: 1.5rem;
  --game-y-margin: 5rem;
  --max-col-width: calc((100vh - var(--game-y-margin) * 2 - var(--header-height) - var(--body-y-padding) * 2 - (${props =>
      props.numCols - 1}) * var(--grid-gap)) / ${props => props.numCols});
  ...
  grid-template-columns: repeat(${props =>
      props.numCols}, minmax(5rem, var(--max-col-width)));
...
export default class Game extends React.Component {
    ...
    compareTiles() {
        const tileAContent = this.state.selectedA.getElementsByClassName(
            'tile__back'
        )[0].textContent;
        const tileBContent = this.state.selectedB.getElementsByClassName(
            'tile__back'
        )[0].textContent;
        ...
        return (
            <Layout>
                <GameWrapper numCols={this.state.numCols}>
                    {this.context.tiles &&
                        this.context.tiles.map(
                            ({ tile, id, memberOfMatch }) => (
                                <Tile
                                    active={
                                        (this.state.selectedA &&
                                            +this.state.selectedA.dataset.id ===
                                                id) ||
                                        (this.state.selectedB &&
                                            +this.state.selectedB.dataset.id ===
                                                id)
                                    }
                                    key={id}
                                    dataId={id}
