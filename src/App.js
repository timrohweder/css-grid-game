...
class App extends Component {
    decrementTries = callback => {
        this.setState(
            state => ({
                triesRemaining:
                    state.triesRemaining !== null
                        ? --state.triesRemaining
                        : state.triesToStart - 1
            }),
            callback ? callback : () => {}
        );
    };
    ...
    createTiles = () => {
        ...
        while (tilesFodder.length < Math.pow(this.state.numCols, 2) / 2) {
            i = Math.floor(
                Math.random() * (Math.pow(this.state.numCols, 2) / 2) + 1
            );
        ...
        [...tilesFodder, ...tilesFodder].forEach(tile => {
            while (true) {
                randomIndex = Math.floor(
                    Math.random() * Math.pow(this.state.numCols, 2)
                );
                if (
                    tiles[randomIndex] === undefined ||
